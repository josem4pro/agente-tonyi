...
from searxng_adapter import searxng_scholar, SEARXNG_URL
...
class Scholar(BaseTool):
    ...
    def _format_scholar_results(self, results, query):
        """Format Serper-compatible scholar results dict into markdown string."""
        try:
    ...
    def google_scholar_with_serp(self, query: str):
        ...
        if USE_SEARXNG:
            results = searxng_scholar(query)
            return self._format_scholar_results(results, query)
