...
from gpu_preflight import run_preflight, post_flight, PreflightError
import time
import math
...
if __name__ == "__main__":
    ...
    # Define ports (single GPU setup uses port 6001 only)
    planning_ports = list(map(int, os.environ.get("VLLM_PORTS", "6001").split(",")))
    # Round-robin state
    planning_rr_idx = 0
    summary_rr_idx = 0
    ...
    if not tasks_to_run_all:
        ...
        try:
            run_preflight(
                ...
                target_context_len=int(os.environ.get("MAX_MODEL_LEN", "30720")),
            )
            session_start = time.time()
            atexit.register(post_flight, start_time=session_start)
